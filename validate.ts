...
async function validateLicenses(): Promise<void> {
    try {
        ...
        if (valid) {
            console.log('✅ Validation successful! licenses.json is valid according to schema.json');
...
function performCustomValidations(data: any): void {
    ...
    if (errors.length === 0) {
        ...
        console.log(`\nSummary:`);
        console.log(`  - ${data.proprietary_licenses.length} proprietary licenses defined`);
